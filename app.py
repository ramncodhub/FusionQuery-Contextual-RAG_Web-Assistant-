...
st.title("⚡ AI Assistant with RAG & Web Fallback")
st.caption(
    "Auto-routes between Document Context & Web Search | Maintains Chat History | Evaluates Outputs"
)
...
def render_evaluation_expander(context_type: str, eval_data: dict):
    """Helper function to render evaluation metrics consistently."""
    with st.expander("📊 AI Evaluation Metrics"):
...
# Sidebar for PDF & Database Management
with st.sidebar:
    ...
    if uploaded_file is not None and st.button("Process Document", type="primary"):
        with st.spinner("Indexing PDF into FAISS Vector DB..."):
            ...
            try:
                ...
                st.success("PDF indexed! FAISS index updated.")
    ...
    if st.button("🗑️ Remove Document & Clear Vector DB"):
        clear_vectorstore()
        st.success("Vector index cleared completely!")
        st.rerun()

    # Chat Memory Reset Button
...
# Chat Input & Streamlit Processing Loop
if prompt := st.chat_input("Ask a question about your PDF or general web topic..."):
